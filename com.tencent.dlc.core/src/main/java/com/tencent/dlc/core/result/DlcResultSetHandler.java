...
/**
 * Handles driver-specific quirks of the Tencent DLC JDBC result set.
 *
 * <p>Some DDL statements (e.g. {@code DESCRIBE TABLE}) return a {@link ResultSet}
 * whose {@link ResultSet#getMetaData()} is {@code null}. Wrapping the result set
 * with this helper ensures downstream consumers always receive a usable metadata
 * object instead of throwing a {@link NullPointerException}.</p>
 *
 * <p>This logic is shared between the DBeaver plugin and the standalone CLI.</p>
 */
public final class DlcResultSetHandler {

    private DlcResultSetHandler() {
        // utility class
    }

    /**
     * Wraps the given result set so that {@link ResultSet#getMetaData()} never
     * returns {@code null}. If the underlying metadata is {@code null}, a minimal
     * one-column metadata object is returned.
     *
     * @param resultSet the result set to wrap; may be {@code null}
     * @return a wrapped result set, or {@code null} if the input was {@code null}
     */
    public static ResultSet safeMetaData(ResultSet resultSet) {
        if (resultSet == null) {
            return null;
        }
        return new SafeMetaDataResultSet(resultSet);
    }

    /**
     * Reads the first column of every remaining row and returns the values as an
     * array. This is useful when no metadata is available and callers still want
     * to show something meaningful to the user.
     *
     * @param resultSet the result set to drain
     * @return the first-column values, never {@code null}
     * @throws SQLException if a database access error occurs
     */
    public static Object[] readFirstColumn(ResultSet resultSet) throws SQLException {
    ...
    private static final class SafeMetaDataResultSet implements ResultSet {

        private final ResultSet delegate;
        private ResultSetMetaData cachedMetaData;

        SafeMetaDataResultSet(ResultSet delegate) {
        ...
        public ResultSetMetaData getMetaData() throws SQLException {
            ...
            if (metaData == null) {
                cachedMetaData = new DlcSingleColumnMetaData();
            } else {
